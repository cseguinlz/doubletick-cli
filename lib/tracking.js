...
const PIXEL_BASE = 'https://europe-west1-gmailtrack-prod.cloudfunctions.net/api';
...
export function injectPixel(html, trackingId) {
  ...
  const pixel = `<img src="${PIXEL_BASE}/img?t=${trackingId}&r=${rand}" width="1" height="1" style="border:0;width:1px;height:1px;overflow:hidden;" alt="">`;
...
export async function registerTrack({ trackingId, recipientEmail, emailSubject }) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      throw new Error('Weekly tracking limit reached. Upgrade to Pro for unlimited tracking: https://doubletickr.com/#pricing');
    }
